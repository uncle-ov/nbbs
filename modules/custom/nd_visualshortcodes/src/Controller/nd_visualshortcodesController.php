...
class nd_visualshortcodesController extends ControllerBase implements ContainerInjectionInterface
{
  ...
  public function nd_visualshortcodes_ajax_backend_layout()
  {
    ...
    $template_path = \Drupal::service('extension.list.module')->getPath('nd_visualshortcodes') . '/templates/nd_visualshortcodes.html.twig';
    $template_class = $this->twig->getTemplateClass($template_path);
    ...
    $template = $this->twig->loadTemplate($template_class, $template_path);

    $markup = $template->render($render_array);
  ...
  public function nd_visualshortcodes_media_upload_image()
  {
    ...
    foreach ($entitys as $file) {
      $img = File::load($file);
      if ($img) {
        if (!\Drupal::service('file_system')->getDestinationFilename($img->getFileUri(), \Drupal\Core\File\FileSystemInterface::EXISTS_ERROR)) {
  ...
  function nd_visualshortcodes_ajax_shortcodes_list()
  {
    ...
    $template_path = \Drupal::service('extension.list.module')->getPath('nd_visualshortcodes') . '/templates/filtre-popup-list.html.twig';
    $template_class = $this->twig->getTemplateClass($template_path);
  ...
  function nd_visualshortcodes_ajax_icons_autocomplete($type = 'all', $str = '')
  {
    $icons = array();
